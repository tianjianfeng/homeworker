...
class TaskRepository(val xa: Transactor[IO]) extends Repository[IO, Task]:
  ...
  def create(task: Task): IO[Task] =
    sql"""
      INSERT INTO tasks (title, description, category, points, user_id, due_date, status, created_at, updated_at)
      VALUES (${task.title}, ${task.description}, ${task.category}, ${task.points}, 
        ${task.userId}, ${task.dueDate}, ${task.status}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  ...
  def findById(id: Long): IO[Option[Task]] =
    sql"""
      SELECT id, title, description, category, status,
        points, user_id, due_date, created_at, updated_at
